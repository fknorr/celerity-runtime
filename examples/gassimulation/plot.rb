#!/usr/bin/env ruby
# Usage: ruby plot.rb [gassimulation_result.bin [-keep]]
...
PREFIX = "#{Dir.tmpdir()}/gassimulation_plot_"

keep_pngs = ARGV.size>1 && ARGV[1] == "-keep" 

filename = ARGV.size>0 ? ARGV[0] : "gassimulation_result.bin"
...
ffmpeg_output = `cat #{image_string} | ffmpeg -y -framerate 60 -f image2pipe -i - -c:v libvpx-vp9 -crf 15 -b:v 0 -pix_fmt yuv444p gassimulation.mp4 2>&1`
if !$?.success?
