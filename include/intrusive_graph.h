...
namespace detail {
	...
	class intrusive_graph_node {
	  ...
	  public:
		...
		void remove_conflict(T* const node) {
			constexpr auto remove_conflict_direction = [](intrusive_graph_node* const from, intrusive_graph_node* const to) {
				const auto remove_begin = std::remove_if(from->conflicts.begin(), from->conflicts.end(), [=](const conflict& cf) { return cf.node == to; });
				from->conflicts.erase(remove_begin, from->conflicts.end());
			};
	  ...
	  private:
		...
		gch::small_vector<conflict> conflicts;
