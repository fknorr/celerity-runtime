...
namespace detail {
	...
	class task : public intrusive_graph_node<task> {
	  ...
	  private:
		...
		// TODO I believe that `struct task` should not store command_group_launchers, task_promise or other state that is related to execution instead of
		...
		std::unique_ptr<task_promise> m_promise;
