...
class side_effect {
  public:
	...
	explicit side_effect(const host_object<T>& object, handler& cgh, detail::side_effect_order_tag<Order> = {}) : object{object} {
		detail::add_side_effect_requirement(object.get_id(), cgh, order);
	}

	object_type& operator*() const { return *object.get_object(); }
	object_type* operator->() const { return object.get_object(); }
