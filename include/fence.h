...
class buffer_fence_promise : public detail::fence_promise {
  public:
	...
	void fulfill() override {
		...
		const auto bid = get_buffer_id(m_buffer);
		const auto mode = access_mode::read;
		const auto access_info = bm.begin_host_buffer_access<DataT, Dims>(bid, mode, range_cast<3>(m_subrange.range), id_cast<3>(m_subrange.offset));
		...
		memcpy_strided(access_info.ptr, data.get(), sizeof(DataT), range_cast<Dims>(access_info.backing_buffer_range),
		    m_subrange.offset - id_cast<Dims>(access_info.backing_buffer_offset), m_subrange.range, {}, m_subrange.range);
		bm.end_buffer_access(
		    buffer_manager::data_location{}.set(bm.get_host_memory_id()), bid, mode, range_cast<3>(m_subrange.range), id_cast<3>(m_subrange.offset));
