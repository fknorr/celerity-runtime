...
class unique_frame_ptr : private std::unique_ptr<Frame, std::function<void(Frame*)>> {
  ...
  public:
	...
	unique_frame_ptr(from_size_bytes_tag, size_t size_bytes)
	    : impl(make_frame(size_bytes), [size_bytes](Frame* frame) { delete_frame(frame, size_bytes); }), m_size_bytes(size_bytes) {}
