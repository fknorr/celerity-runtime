...
template <int Dims>
async_event memcpy_strided_device(sycl::queue& queue, const void* source_base_ptr, void* target_base_ptr, size_t elem_size, const range<Dims>& source_range,
    const id<Dims>& source_offset, const range<Dims>& target_range, const id<Dims>& target_offset, const range<Dims>& copy_range, void* HACK_backend_context) {
