...
class queue {
  public:
	...
	virtual std::pair<void*, std::unique_ptr<event>> malloc(memory_id where, size_t size, size_t alignment) = 0;

	virtual std::unique_ptr<event> free(memory_id where, void* allocation) = 0;
	...
	virtual std::unique_ptr<event> launch_kernel(device_id did, const sycl_kernel_launcher& launcher, const subrange<3>& execution_range) = 0;
};
