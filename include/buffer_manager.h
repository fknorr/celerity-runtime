	  ...
	  public:
		...
		buffer_id register_buffer(range<3> range, const DataT* host_init_ptr = nullptr) {
			...
			const bool is_host_initialized = host_init_ptr != nullptr;
			{
				...
				m_buffer_infos.emplace(bid, buffer_info{Dims, range, sizeof(DataT), alignof(DataT), is_host_initialized, host_init_ptr, {},
				                                std::move(device_factory), std::move(host_factory)});
