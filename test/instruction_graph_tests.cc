...
TEST_CASE("instruction graph") {
	...
	auto buf2 = dctx.create_buffer(test_range);
	auto ho = dctx.create_host_object();

	dctx.device_compute<class UKN(producer)>(test_range).discard_write(buf1, acc::one_to_one()).submit();
	dctx.device_compute<class UKN(gather)>(test_range).read(buf1, acc::all()).discard_write(buf2, acc::one_to_one()).submit();
	dctx.device_compute<class UKN(consumer)>(test_range).read(buf2, acc::all()).submit();

	const size_t num_devices = 2;
	instruction_graph_generator iggen(dctx.get_task_manager(), num_devices);
	iggen.register_buffer(buf1.get_id(), range<3>(256, 1, 1)); // TODO have an idag_test_context to do this
	iggen.register_buffer(buf2.get_id(), range<3>(256, 1, 1));
