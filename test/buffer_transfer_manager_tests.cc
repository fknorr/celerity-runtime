...
// NOCOMMIT TODO: Should this be in test/system ?
// NOCOMMIT TODO: Run in CI (what node configuration?)
...
struct buffer_transfer_manager_testspy {
	...
	static size_t get_request_transfer_count(const buffer_transfer_manager& btm, const buffer_id bid, const transfer_id trid) {
		REQUIRE(btm.m_requests.count(std::pair{bid, trid}) == 1);
		return btm.m_requests.at(std::pair{bid, trid}).m_transfers.size();
	}
};
} // namespace celerity::detail
...
void poll_until(buffer_transfer_manager& btm, Predicate p) {
	...
	while(!p()) {
		...
		if(std::chrono::steady_clock::now() - start_ts > std::chrono::seconds(5)) {
			FAIL_CHECK("Polled without change for 5 seconds, likely deadlocked - aborting.");
			MPI_Abort(MPI_COMM_WORLD, 1); // Kill other ranks that might be deadlocked on MPI_Barrier
		}
	}
}
...
class buffer_transfer_manager_fixture : public test_utils::mpi_fixture {
  public:
	buffer_transfer_manager_fixture() : m_config(nullptr, nullptr) {
		...
		m_buffer_transfer_mngr = std::make_unique<buffer_transfer_manager>(m_num_ranks, *m_buffer_mngr, m_reduction_mngr);
	}

	// TODO: Require exact or at least?
	void require_num_ranks(const size_t num_ranks) const {
		if(m_num_ranks != static_cast<int>(num_ranks)) { SKIP("Test requires " << num_ranks << " ranks, have " << m_num_ranks); }
	}
	...
	buffer_id create_buffer(const size_t range_1d) {
		...
		auto info = m_buffer_mngr->access_host_buffer<char, 1>(bid, access_mode::discard_write, {{}, range<1>(range_1d)});
		std::memset(info.ptr, m_rank + 1, range_1d);
		return bid;
	}
	...
	void verify_buffer_ranges(const buffer_id bid, const std::vector<std::pair<subrange<1>, node_id>>& owned_by) {
		for(auto& [sr, nid] : owned_by) {
			auto info = m_buffer_mngr->access_host_buffer<char, 1>(bid, access_mode::read, {sr.offset, sr.range});
...
TEST_CASE_METHOD(buffer_transfer_manager_fixture, "a push from every peer is required to satisfy a reduction await push") {
	...
	const transfer_id trid = 123;
	const reduction_id rid = rm.create_reduction<int, 1>(bid, std::plus<>{}, 0);

	const auto sr = subrange<3>({0, 0, 0}, {1, 1, 1});
	...
	std::shared_ptr<const buffer_transfer_manager::transfer_handle> rank0_handle;
	if(get_rank() == 0) {
		poll_until(btm, [&]() {
			return buffer_transfer_manager_testspy::get_request_count(btm) == 1
			       && buffer_transfer_manager_testspy::get_request_transfer_count(btm, bid, trid) == 2;
		});
		rank0_handle = btm.await_push(trid, bid, subrange_to_grid_box(sr), rid);
		CHECK_FALSE(rank0_handle->complete);
	}
	...
	if(get_rank() == 0) {
		poll_until(btm, [&]() { return rank0_handle->complete; });
	}
