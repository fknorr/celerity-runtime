	  ...
	  public:
		...
		range<Dims> get_backing_buffer_range(detail::buffer_id bid, access_target tgt, cl::sycl::range<Dims> range, cl::sycl::id<Dims> offset) {
			if(tgt == access_target::host) {
				auto info =
				    m_bm->begin_host_buffer_access<DataT, Dims>(bid, cl::sycl::access::mode::read, detail::range_cast<3>(range), detail::id_cast<3>(offset));
				return detail::range_cast<Dims>(info.backing_buffer_range);
			}
			auto info =
			    m_bm->begin_device_buffer_access<DataT, Dims>(bid, cl::sycl::access::mode::read, detail::range_cast<3>(range), detail::id_cast<3>(offset));
			return detail::range_cast<Dims>(info.backing_buffer_range);
		}
		...
		void buffer_for_each(detail::buffer_id bid, access_target tgt, cl::sycl::range<Dims> range, cl::sycl::id<Dims> offset, Callback cb) {
			...
			if(tgt == access_target::host) {
				auto info = m_bm->begin_host_buffer_access<DataT, Dims>(bid, Mode, range3, offset3);
				...
				for(size_t i = offset3[0]; i < offset3[0] + range3[0]; ++i) {
					for(size_t j = offset3[1]; j < offset3[1] + range3[1]; ++j) {
						for(size_t k = offset3[2]; k < offset3[2] + range3[2]; ++k) {
							...
							cb(detail::id_cast<Dims>(global_idx), reinterpret_cast<DataT*>(info.ptr)[linear_idx]);
						}
					}
				}
			}
			...
			if(tgt == access_target::device) {
				auto info = m_bm->begin_device_buffer_access<DataT, Dims>(bid, Mode, range3, offset3);
				...
				get_device_queue()
				    .submit([&](cl::sycl::handler& cgh) {
					    ...
					    cgh.parallel_for<detail::bind_kernel_name<KernelName>>(range, [=](cl::sycl::id<Dims> global_idx) {
						    ...
						    cb(global_idx, reinterpret_cast<DataT*>(ptr)[detail::get_linear_index(buf_range, local_idx)]);
					    });
				    })
				    .wait();
			}
		...
		ReduceT buffer_reduce(detail::buffer_id bid, access_target tgt, cl::sycl::range<Dims> range, cl::sycl::id<Dims> offset, ReduceT init, Operation op) {
			...
			if(tgt == access_target::host) {
				auto info = m_bm->begin_host_buffer_access<DataT, Dims>(bid, cl::sycl::access::mode::read, range3, offset3);
				...
				for(size_t i = offset3[0]; i < offset3[0] + range3[0]; ++i) {
					for(size_t j = offset3[1]; j < offset3[1] + range3[1]; ++j) {
						for(size_t k = offset3[2]; k < offset3[2] + range3[2]; ++k) {
							...
							const cl::sycl::id<3> local_idx = global_idx - info.backing_buffer_offset;
							const size_t linear_idx = local_idx[0] * buf_range[1] * buf_range[2] + local_idx[1] * buf_range[2] + local_idx[2];
							result = op(detail::id_cast<Dims>(global_idx), result, reinterpret_cast<DataT*>(info.ptr)[linear_idx]);
						}
					}
				}
				return result;
			...
			auto info = m_bm->begin_device_buffer_access<DataT, Dims>(bid, cl::sycl::access::mode::read, range3, offset3);
			...
			get_device_queue()
			    .submit([&](cl::sycl::handler& cgh) {
				    auto ptr = reinterpret_cast<DataT*>(info.ptr);
		...
		accessor<DataT, Dims, Mode, target::device> get_device_accessor(
			...
			auto buf_info = m_bm->begin_device_buffer_access<DataT, Dims>(bid, Mode, detail::range_cast<3>(range), detail::id_cast<3>(offset));
			return detail::accessor_testspy::make_device_accessor<DataT, Dims, Mode>(reinterpret_cast<DataT*>(buf_info.ptr),
		...
		accessor<DataT, Dims, Mode, target::host_task> get_host_accessor(
			...
			auto buf_info = m_bm->begin_host_buffer_access<DataT, Dims>(bid, Mode, detail::range_cast<3>(range), detail::id_cast<3>(offset));
			return detail::accessor_testspy::make_host_accessor<DataT, Dims, Mode>(subrange<Dims>(offset, range), reinterpret_cast<DataT*>(buf_info.ptr),
			    detail::id_cast<Dims>(buf_info.backing_buffer_offset), detail::range_cast<Dims>(buf_info.backing_buffer_range),
