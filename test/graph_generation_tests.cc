...
namespace detail {
	...
	TEST_CASE("side effects of different order create appropriate conflict sets", "[task_manager][task-graph][side-effect]") {
		...
		const auto tid_4_epoch = test_utils::build_and_flush(ctx, num_nodes, tm.finish_epoch(epoch_action::shutdown));
