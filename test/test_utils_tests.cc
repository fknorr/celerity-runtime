...
TEST_CASE("command_query::find_all supports various filters", "[command_query]") {
	...
	auto* push0 = cdags[0]->create<push_command>(node_id(1), receive_id(transfer_id(0), buffer_id(0), reduction_id(0)), subrange<3>{});
	auto* push1 = cdags[1]->create<push_command>(node_id(0), receive_id(transfer_id(0), buffer_id(0), reduction_id(0)), subrange<3>{});
