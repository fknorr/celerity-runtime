...
namespace detail {
	...
	struct task_manager_testspy {
		static std::optional<task_id> get_emerging_horizon(task_manager& tm) { return tm.emerging_horizon; }
	...
	class cdag_test_context {
	  public:
		...
		detail::task_id build_task_horizons() {
			const auto most_recently_generated_task_horizon = detail::task_manager_testspy::get_emerging_horizon(get_task_manager());
