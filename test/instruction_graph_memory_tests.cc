...
TEST_CASE("host staging only writes to device allocations that are not yet up to date", "[instruction_graph_generator][instruction-graph][memory]") {
	...
	CHECK(std::set<memory_id>{all_unstage_copies_2[0]->dest_allocation_id.get_memory_id(), all_unstage_copies_2[1]->dest_allocation_id.get_memory_id()}
	      == std::set<memory_id>{first_device_memory_id + 2, first_device_memory_id + 3});
