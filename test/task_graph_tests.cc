...
namespace detail {
	...
	TEST_CASE("task horizons are being generated with correct dependencies", "[task_manager][task-graph][task-horizon]") {
		...
		auto emerging_horizon = task_manager_testspy::get_emerging_horizon(tm);
		CHECK(!emerging_horizon.has_value());

		const auto tid_c = test_utils::add_host_task(tm, on_master_node, [&](handler& cgh) { buf_a.get_access<access_mode::read>(cgh, fixed<1>({0, 128})); });

		emerging_horizon = task_manager_testspy::get_emerging_horizon(tm);
		REQUIRE(emerging_horizon.has_value());
		CHECK(*emerging_horizon == tid_c + 1);
		CHECK(task_manager_testspy::get_num_horizons(tm) == 1);

		auto horizon_dependencies = tm.get_task(*emerging_horizon)->get_dependencies();
		...
		// emerging horizon is always part of the active task front
		expected_dependency_ids.insert(*emerging_horizon);
		...
		emerging_horizon = task_manager_testspy::get_emerging_horizon(tm);
		REQUIRE(emerging_horizon.has_value());
		CHECK(*emerging_horizon == tid_d + 1);
		CHECK(task_manager_testspy::get_num_horizons(tm) == 2);

		horizon_dependencies = tm.get_task(*emerging_horizon)->get_dependencies();
	...
	TEST_CASE("task horizons update previous writer data structure", "[task_manager][task-graph][task-horizon]") {
		...
		auto horizon = task_manager_testspy::get_emerging_horizon(tm);
		CHECK(task_manager_testspy::get_num_horizons(tm) == 1);
	...
	TEST_CASE("previous task horizon is used as last writer for host-initialized buffers", "[task_manager][task-graph][task-horizon]") {
		...
		// Create a bunch of tasks to trigger horizon cleanup
		{
			...
			for(int i = 0; i < 8; ++i) {
				...
				const auto emerging_horizon = task_manager_testspy::get_emerging_horizon(tm);
				if(emerging_horizon && *emerging_horizon > last_executed_horizon) {
					last_executed_horizon = *emerging_horizon;
		...
		const auto emerging_horizon = task_manager_testspy::get_emerging_horizon(tm);
		REQUIRE(emerging_horizon);
		INFO("previous horizon is being used");
		CHECK(new_last_writer->get_id() < *emerging_horizon);
