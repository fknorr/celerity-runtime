...
namespace detail {
	...
	TEST_CASE("collective groups create closed conflict sets separated by epochs", "[task_manager][task-graph]") {
		...
		const auto tid_epoch_b = tm.finish_epoch(epoch_action::barrier);
		...
		const auto tid_epoch_c = tm.finish_epoch(epoch_action::shutdown);
	...
	TEST_CASE("side effects of different order create appropriate conflict sets", "[task_manager][task-graph][side-effect]") {
		...
		const auto tid_4_epoch = tm.finish_epoch(epoch_action::shutdown);
