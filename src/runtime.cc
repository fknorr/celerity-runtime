...
#error "unknown SYCL implementation"
	...
	void runtime::shutdown(buffer_capture_map capture_map, side_effect_map side_effect_map) noexcept {
		assert(is_active);
		is_shutting_down = true;

		const auto final_epoch = task_mngr->finish_epoch(epoch_action::shutdown, std::move(capture_map), std::move(side_effect_map));
	...
	void runtime::sync(buffer_capture_map capture_map, side_effect_map side_effect_map) {
		const auto epoch = task_mngr->finish_epoch(epoch_action::barrier, std::move(capture_map), std::move(side_effect_map));
