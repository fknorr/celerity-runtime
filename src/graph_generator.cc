...
namespace detail {
	...
	void graph_generator::build_task(const task& tsk, const std::vector<graph_transformer*>& transformers) {
		...
		case task_type::inclusive_scan: generate_inclusive_scan_commands(tsk); break;
		}
	...
	void graph_generator::generate_gather_commands(const task& tsk) {
		...
		for(size_t nid = 0; nid < m_num_nodes; ++nid) {
			...
			if(pcmd != producer_cmds.end()) {
				if(const auto ecmd = dynamic_cast<const execution_command*>(*pcmd)) {
					producer_sr = ecmd->get_execution_range();
				} else if(const auto iscmd = dynamic_cast<const inclusive_scan_command*>(*pcmd)) {
					producer_sr = iscmd->get_subrange();
				} else {
	...
	void graph_generator::generate_inclusive_scan_commands(const task& tsk) {
		assert(tsk.get_type() == task_type::inclusive_scan);

		const auto sr = subrange<3>{tsk.get_global_offset(), tsk.get_global_size()};
		m_cdag.create<inclusive_scan_command>(0, tsk.get_id(), sr);
	}
	...
	void graph_generator::process_task_data_requirements(const task& tsk) {
		...
		for(auto* cmd : task_commands) {
			...
			execution_command* ecmd = nullptr;
			inclusive_scan_command* iscmd = nullptr;
			if((ecmd = dynamic_cast<execution_command*>(cmd)) || (iscmd = dynamic_cast<inclusive_scan_command*>(cmd))) {
				if(ecmd) {
					...
					requirements = get_buffer_requirements_for_mapped_access(tsk, iscmd->get_subrange(), tsk.get_global_size());
				}
