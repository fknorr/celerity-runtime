...
namespace detail {
	...
	[[gnu::noinline]] void graph_generator::generate_epoch_commands(const task* const tsk) {
		...
		for(node_id nid = 0; nid < num_nodes; ++nid) {
			...
			node.emerging_horizon = std::nullopt;
	...
	[[gnu::noinline]] void graph_generator::generate_horizon_commands(const task* const tsk) {
		...
		for(node_id nid = 0; nid < num_nodes; ++nid) {
			...
			if(node.emerging_horizon) {
				if(min_new_epoch) {
					min_new_epoch = std::min(*min_new_epoch, *node.emerging_horizon);
				} else {
					min_new_epoch = node.emerging_horizon;
				}
				node.set_epoch_for_new_commands(*node.emerging_horizon);
			}
			node.emerging_horizon = cid;
