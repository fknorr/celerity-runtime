...
namespace detail {
	...
	void graph_generator::per_node_data::set_current_epoch(const command_id epoch) {
		...
		for(auto& [_, cg] : collective_groups) {
	...
	void graph_generator::build_task(task_id tid, const std::vector<graph_transformer*>& transformers) {
		...
		if(tsk->get_type() == task_type::HORIZON || tsk->get_type() == task_type::EPOCH) {
			...
			for(size_t nid = 0; nid < num_nodes; ++nid) {
				...
				auto& collective_group = node_data.at(nid).collective_groups[cgid];
				if(collective_group.last_collective_command_or_epoch) {
					cdag.add_dependency(cmd, cdag.get(*collective_group.last_collective_command_or_epoch), dependency_kind::TRUE_DEP,
				...
				if(order == experimental::side_effect_order::sequential) {
					...
					for(const auto [concurrent_cid, concurrent_order] : ho.active_concurrent_set) {
						if(order == experimental::side_effect_order::exclusive || concurrent_order == experimental::side_effect_order::exclusive) {
							cmd->add_conflict({cdag.get(concurrent_cid), conflict_origin::side_effect_order});
						}
					}
