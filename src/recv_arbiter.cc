...
void recv_arbiter::begin_individual_recv(waiting_for_communication& state, const node_id source, const pilot_message& pilot) {
	const auto [offset_in_buffer, payload_range] = pilot.box.get_subrange();
	...
	state.active_individual_recvs.push_back(m_comm->receive_payload(source, pilot.tag, state.allocation_base, stride));
...
bool recv_arbiter::forget_if_complete(const transfer_id trid, const buffer_id bid) {
	...
	const auto last_incomplete = std::remove_if(state.active_individual_recvs.begin(), state.active_individual_recvs.end(),
	    [](const std::unique_ptr<communicator::event>& evt) { return evt->is_complete(); });
	state.active_individual_recvs.erase(last_incomplete, state.active_individual_recvs.end());
