...
namespace detail {
	...
	backend::async_event buffer_manager::get_buffer_data(buffer_id bid, const subrange<3>& sr, void* out_linearized) {
		...
		while(!pending_slow_transfers.empty()) {
			if(const auto done_it = std::find_if(pending_slow_transfers.begin(), pending_slow_transfers.end(), //
				...
				switch(buffer_info.dimensions) {
				case 1:
					memcpy_strided_host(temp.get_pointer(), out_linearized, buffer_info.element_size, range_cast<1>(part_sr.range), id<1>(0),
					    range_cast<1>(sr.range), id_cast<1>(part_sr.offset - sr.offset), range_cast<1>(part_sr.range));
					break;
				case 2:
					memcpy_strided_host(temp.get_pointer(), out_linearized, buffer_info.element_size, range_cast<2>(part_sr.range), id<2>(0, 0),
					    range_cast<2>(sr.range), id_cast<2>(part_sr.offset - sr.offset), range_cast<2>(part_sr.range));
					break;
				case 3:
					memcpy_strided_host(temp.get_pointer(), out_linearized, buffer_info.element_size, part_sr.range, id<3>(0, 0, 0), sr.range,
	...
	buffer_manager::access_info buffer_manager::access_device_buffer(const memory_id mid, buffer_id bid, access_mode mode, const subrange<3>& sr) {
#if TRACY_ENABLE
		...
		const auto die = [&](const size_t allocation_size_bytes) {
			std::string msg = fmt::format("Unable to allocate buffer {} of size {} on device {} (memory {}).\n", bid, allocation_size_bytes,
			    device_queue.get_id(), device_queue.get_memory_id());
			...
			fmt::format_to(std::back_inserter(msg), "Total usage: {} / {} bytes ({:.1f}%).\n", total_bytes, device_queue.get_global_memory_total_size_bytes(),
			    100 * static_cast<double>(total_bytes) / static_cast<double>(device_queue.get_global_memory_total_size_bytes()));
		...
		if(!existing_buf.is_allocated()) {
			...
			if(!can_allocate(device_queue.get_memory_id(), allocation_size_bytes)) {
			...
			replacement_buf = backing_buffer{m_buffer_infos.at(bid).construct_device(sr.range, device_queue, m_cuda_copy_streams.at(mid - 1).get()), sr.offset};
			...
			if(can_allocate(device_queue.get_memory_id(), allocation_size_bytes)) {
				...
				if(!can_allocate(device_queue.get_memory_id(), allocation_size_bytes - (existing_buf.storage->get_range().size() * element_size))) {
					// Final attempt: Check if we can create a new buffer with the requested size if we spill everything else to the host.
					if(can_allocate(device_queue.get_memory_id(), sr.range.size() * element_size, existing_buf.storage->get_range().size() * element_size)) {
						CELERITY_WARN("Spilling buffer {} on memory {}, existing size {}, to enable acecss {}\n", bid, mid,
						    subrange<3>(existing_buf.offset, existing_buf.storage->get_range()), subrange<3>(sr.offset, sr.range));
						spill_to_host = true;
					} else {
				...
				retain_region.scanByBoxes([&](const GridBox<3>& box) {
					...
					access_host_buffer_impl(bid, access_mode::read, sr).pending_transfers.wait();
				});
				...
				replacement_buf = backing_buffer{
				    m_buffer_infos.at(bid).construct_device(spill_to_host ? sr.range : info.new_range, device_queue, m_cuda_copy_streams.at(mid - 1).get()),
				    spill_to_host ? sr.offset : info.new_offset};
		...
		auto [coherent_buf, pending_transfers] =
		    make_buffer_subrange_coherent(mid, bid, mode, std::move(existing_buf), {sr.offset, sr.range}, std::move(replacement_buf));
	...
	buffer_manager::access_info buffer_manager::access_host_buffer_impl(const buffer_id bid, const access_mode mode, const subrange<3>& sr) {
		...
		auto [coherent_buf, pending_transfers] = make_buffer_subrange_coherent(
		    m_local_devices.get_host_memory_id(), bid, mode, std::move(existing_buf), {sr.offset, sr.range}, std::move(replacement_buf));
		existing_buf = std::move(coherent_buf);
