...
namespace detail {
	...
	buffer_manager::access_info buffer_manager::access_device_buffer(const memory_id mid, buffer_id bid, access_mode mode, const subrange<3>& sr) {
		...
		if(!existing_buf.is_allocated()) {
			...
			replacement_buf = backing_buffer{m_buffer_infos.at(bid).construct_device(sr.range, device_queue), sr.offset};
			...
			if(can_allocate(device_queue.get_memory_id(), allocation_size_bytes)) {
				...
				replacement_buf = backing_buffer{m_buffer_infos.at(bid).construct_device(spill_to_host ? sr.range : info.new_range, device_queue),
