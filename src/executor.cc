		...
		while(!done || !jobs.empty()) {
			...
			for(auto it = jobs.begin(); it != jobs.end();) {
				...
				for(const auto& d : job_handle.dependents) {
					...
					if(jobs[d].unsatisfied_dependencies == 0) { ready_jobs.insert(d); }
				}
			...
			if(flag == 1) {
				...
				// TODO we don't really need the conflict vector explicitly in the executor
