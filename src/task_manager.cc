...
namespace detail {
	...
	task_id task_manager::generate_horizon_task() {
		...
		task_id tid;
		{
			...
			tid = get_new_tid();
			emerging_horizon_critical_path_length = max_pseudo_critical_path_length;
			const auto previous_horizon = emerging_horizon;
			emerging_horizon = reduce_execution_front(task::make_horizon_task(tid));
	...
	task_id task_manager::generate_epoch_task(epoch_action action) {
		...
		task_id tid;
		{
			...
			emerging_horizon = std::nullopt; // this horizon is now behind the epoch_for_new_tasks, so it will never become an epoch itself
			emerging_horizon_critical_path_length = max_pseudo_critical_path_length; // the explicit epoch resets the need to create horizons
		}
