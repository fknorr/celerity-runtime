...
namespace detail {
	...
	void task_manager::notify_epoch_reached(const task_id epoch_tid) { m_task_graph.delete_before_epoch(epoch_tid); }
	...
	void task_manager::invoke_callbacks(const task* tsk) const {
		if(m_delegate != nullptr) { m_delegate->notify_task_created(tsk); }
