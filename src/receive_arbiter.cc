...
void receive_arbiter::begin_receive(const transfer_id& trid, void* const allocation, const box<3>& allocated_box, const size_t elem_size) {
	auto& transfer = m_transfers[trid]; // allow default-insert
...
receive_arbiter::event receive_arbiter::await_receive(const transfer_id& trid, const region<3>& awaited_region) {
	auto& transfer = m_transfers.at(trid);
...
void receive_arbiter::end_receive(const transfer_id& trid) {
	...
	auto& transfer = m_transfers.at(trid); // must exist
...
void receive_arbiter::poll_communicator() {
	...
	for(const auto& pilot : m_comm->poll_inbound_pilots()) {
		auto& transfer = m_transfers[pilot.message.trid]; // allow default-insert
		const auto region_it = std::find_if(transfer.regions.begin(), transfer.regions.end(),
