...
namespace detail {
	...
	std::pair<celerity::detail::command_type, std::string> epoch_job::get_description(const command_pkg& pkg) {
		return std::make_pair(command_type::EPOCH, "EPOCH");
	}

	bool epoch_job::execute(const command_pkg& pkg, std::shared_ptr<logger> logger) {
